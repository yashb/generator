...
echo 'List all classes in the example source file: '. $file . PHP_EOL;

foreach ($project->getNamespaces() as $namespaces) {
    echo '- ' . $namespaces->getFqsen() . PHP_EOL;
}

/** @var \phpDocumentor\Reflection\Php\Class_ $class */
foreach ($project->getFiles()[$file]->getClasses() as $class) {
    echo '- ' . $class->getFqsen() . PHP_EOL;
    
    //list functions
    foreach ($class->getMethods() as $methods) {
        echo '- ' . $methods->getFqsen() . PHP_EOL;
    }
